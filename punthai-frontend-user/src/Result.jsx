...
export const Result = () => {
  ...
  const [generatedItems, setGeneratedItems] = useState([
    { id: 1, category: 'label',  isFavorite: false, label: 'Label 1' },
    { id: 2, category: 'label',  isFavorite: false, label: 'Label 2' },
    { id: 3, category: 'label',  isFavorite: false, label: 'Label 3' },
    { id: 4, category: 'mockup', isFavorite: false, label: 'Mockup 1' },
    { id: 5, category: 'mockup', isFavorite: false, label: 'Mockup 2' },
    { id: 6, category: 'mockup', isFavorite: false, label: 'Mockup 3' },
  ]);
    ...
    <div className="rs-body">
      ...
      <div className="rs-container">
        <aside className={`rs-sidebar ${isSidebarCollapsed ? 'rs-collapsed' : ''}`}>
          ...
          <ul className="rs-menu">
            <li onClick={() => navigate('/project', { state: { projectId } })}>
              <span className="rs-icon"><iconify-icon icon="mdi:view-dashboard-outline"></iconify-icon></span>
              <span className="rs-text">Projects</span>
            </li>
            <li onClick={() => navigate('/brand-dna', { state: { projectId } })}>
              <span className="rs-icon"><iconify-icon icon="mdi:palette-outline"></iconify-icon></span>
              <span className="rs-text">Brand DNA</span>
            </li>
            <li onClick={() => navigate('/create-concept', { state: { projectId } })}>
              <span className="rs-icon"><iconify-icon icon="mdi:lightbulb-outline"></iconify-icon></span>
              <span className="rs-text">Create Concept</span>
            </li>
            <li onClick={() => navigate('/create-logo', { state: { projectId } })}>
              <span className="rs-icon"><iconify-icon icon="mdi:folder-outline"></iconify-icon></span>
              <span className="rs-text">Create Logo</span>
            </li>
            <li onClick={() => navigate('/result', { state: { projectId } })}>
              <span className="rs-icon"><iconify-icon icon="mdi:image-multiple-outline"></iconify-icon></span>
              <span className="rs-text">Create Pictures</span>
            </li>
          </ul>
          <hr className="rs-hr" />
          <ul className="rs-menu">
            <li onClick={() => navigate('/your-projects', { state: { projectId } })}>
              <span className="rs-icon"><iconify-icon icon="mdi:folder-outline"></iconify-icon></span>
              <span className="rs-text">Yours Projects</span>
            </li>
          </ul>
        ...
        <div className="rs-main-content">
          ...
          <div className="rs-tabs-wrapper">
            <div className="rs-tabs">
              <div
                ...
                style={{ left: `${indicatorStyle.left}px`, width: `${indicatorStyle.width}px`, background: indicatorStyle.background }}
              />
          ...
          {/* ══════════════════════════════════
              TAB: PACKAGE — Catalog with Filter
              ══════════════════════════════════ */}
                    ...
                    <div className="rs-logo-actions">
                      ...
                      <button className="rs-action-btn" onClick={() => toggleFavorite(item.id)}>
                        <iconify-icon
                          ...
                          style={{ color: item.isFavorite ? '#D35325' : '#666' }}
                        />
                      </button>
