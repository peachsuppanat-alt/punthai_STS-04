...
import './ConceptSidebarNews.css';
import sidebarBg from '../assets/bg_sidebar.png';

/**
 * ConceptSidebarNews — Sidebar ใช้ร่วมกันทุกหน้าใน project_feature
 *
 * Props:
 ...
 */
export const ConceptSidebarNews = ({ activePage, projectId, onLogout }) => {
  ...
  return (
    <>
      ...
      <aside className={`csnews-sidebar ${isCollapsed ? 'csnews-collapsed' : ''}`}>

        {/* ── เมนูหลัก ── */}
        <ul className="csnews-menu">
          {menuItems.map((item) => (
            <li
              key={item.key}
              className={activePage === item.key ? 'csnews-active' : ''}
              onClick={() => go(item.path)}
              title={isCollapsed ? item.label : ''}
            >
              <span className="csnews-icon">
                <iconify-icon icon={item.icon}></iconify-icon>
              </span>
              <span className="csnews-text">{item.label}</span>
            </li>
          ))}
        </ul>

        <hr className="csnews-hr" />
        ...
        <div className="csnews-bg-illustration">
...
export default ConceptSidebarNews;